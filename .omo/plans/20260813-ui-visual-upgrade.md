...
- [ ] **1.2 收敛内联 `setStyleSheet` 到 `GlobalStyle` getter**
  ...
  - 验收：`rg "setStyleSheet" src/` 数量 < 50，仅保留运行时动态态；`rg "#[0-9A-Fa-f]{6}" src/ -g '!**/GlobalStyle.cpp'` 在 `.cpp` 中的字面量清零或附说明。
...
- **待办**：(1) 用户裁决 A3 收敛路径：批次6 DecisionView 28 处深转换 vs 阈值修订；(2) A4 三视口验证方案确认（扩 ScreenshotTool 或 xcb 手动）；(3) 阶段2（2.1 对比度 / 2.2 图标）启动确认。
