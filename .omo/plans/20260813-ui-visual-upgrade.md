...
- [x] **3.1 补 elevation/阴影 token**（已完成，已验证，2026-08-20，执行记录见下"阶段3·3.1"）
...
- [x] **3.2 克制过渡动画（已取消，2026-08-20 用户裁决）**
  - 取消理由：军事场景本就克制动画，QSS hover 态维持瞬时切换，不引入 `QPropertyAnimation` 过渡。
...
- **阶段3·3.1 深度投影（已完成，已验证，2026-08-20）**：改动六文件=计划 3.1 五文件 + 集成修复 DecisionViewLayout.cpp（超出原五文件清单，系 3.1 像素门禁暴露的缩放缺陷，与 3.1 同批交付）。五文件：design-system.md 新增 §9 深度投影（Elevation token 表：Overlay `0 4px 12px rgba(0,0,0,0.4)` 供态势页 PiP/目标详情浮层底衬、Modal `0 8px 24px rgba(0,0,0,0.5)` 供生成器对话框卡片）+ §9.1 实现方式与已知限制（FloatingShadowLayer 底衬机制/交换模式限制/模态规格/视口缩放规则）；GlobalStyle.h 新增 Elevation 常量组（BlurRadius/OffsetY/ShadowAlpha 两档）；MainWindow.h/cpp 新增 FloatingShadowLayer 阴影底衬（PiP 子树含原生 QVideoWidget，effect 栅格路径无法捕获原生子窗口内容，投影由同几何底衬承载，repositionFloatingWidgets 同步几何、eventFilter Hide 事件同步隐藏）；MosGeneratorDialog 重设计（DEC-GEN-MODAL 无边框化 `Qt::FramelessWindowHint`+`WA_TranslucentBackground`、setFixedSize(568,424)、四周 24/36px 投影泄露区、DEC-GEN-CARD 520x364 挂 Modal 档 QGraphicsDropShadowEffect）。集成修复（DecisionViewLayout.cpp）：(a) QAbstractSpinBox 缩放幂等化——p03@4K 门禁回归 5.167%（428,604px）经 probe4k 四阶段探针+文件级 bisect 实证归因：3.1 MainWindow 深度投影改动触发主窗 show 期 applyViewportScale 以 scale=1.0 提前运行（决策页隐藏 640x480 钳位 1.0），既有 mosFontRole 守卫令 spinbox 缩放仅首跑生效，4K 切页后 spin 卡 30（应 58）、参数栏矮 112px、跑道相应高 112px；修复=spinbox 分支移至 role 守卫前无条件重设高度（幂等化，不回退触发源——布局期重算应天然安全）。(b) 对话框缩放块统一 568x424 基准——旧块按旧设计 520x360×scale setMinimumSize+resize 与新 setFixedSize(568,424) 冲突（1.0 档卡片右 24px/下 28px 被裁、4K 档窗口 1011x700 而卡片停留 1.0 几何），改 setFixedSize(568×s,424×s)+DEC-GEN-CARD setGeometry(24s,24s,520s,364s)，CLOSE 28s/BANNER 28s 缩放维持。验证：MainWindowCapture 构建 100%；探针 4K S4 spin=58/参数栏 442/跑道 1487（与 BEFORE 基线精确一致）、S5 应用内对话框 1104x824+卡片 (46,46,1011,708)；1280 对照 S5 对话框 568x424+卡片 (24,24,520,364)（=设计基准）；门禁 after3 对 before：p03 三视口 1280/1920/4K=0.00%/0.00%/0.00%（4K 回归归零）、p01@1920=0.05%（HUD 时码等既有运行态簇，非本批引入）；ctest 15/17（video_render_capture"target row missing"/stop_select_flicker_repro"no target rows"为既有失败，根因 PatchCore DetectionEngine.cpp L61-63 零检出，与本批无关）。已知限制登记：对话框孤立离屏 grab 中投影黑上黑不可见，纵深以泄露边距几何+effect 挂载结构验证。证据：`/tmp/opencode/uiupgrade-batch7/`（probe4k.cpp 四阶段+S5、after3/、compare3.py、ctest_fix.log、dialog_inapp_*.png）。
- **收官记录（2026-08-21，REQ-010 收官）**：合并 origin/main（0b61106，PR #16 探测阈值校准与文档修正）后全量重链暴露 GNU ld 2.38 链接缺陷：编译器压栈语义的 `push foo@GOTPCREL(%rip)`（R_X86_64_GOTPCRELX）被错误松弛为 `addr32 call foo`，跳入 .data.rel.ro 只读数据，致全部构造 MainWindow 的测试 SIGSEGV（合并后 9/17；MosPlanningController.cpp.o 干净而二进制含 addr32 call 的对照取证定案，ld 2.38 缺陷）。规避：根 CMakeLists.txt `add_link_options(-Wl,--no-relax)`（附证据链注释）；重链后全部可执行 0 处 addr32 call，StartupTest 探针 exit=0、PatchCore/YOLO 模型正常加载。附带修复：mos_ui_closure `generatorResizesAfterFourKTransition` 陈旧期望对齐 3.1 基准（520x360 -> 568x424；3.1 验证期仅构建 MainWindowCapture、测试二进制未重链，合并日全量重建方暴露）。终态：Release 全目标构建 100% + 全量 ctest 15/17（video_render_capture/stop_select_flicker_repro 为主干探测既有失败，PatchCore 零检出）。落账：REQ-009/REQ-010 置 `Implemented`（实现事实入需求文件），注册表索引/NEXT 段与三份 feature 文档状态对齐，去除 feature 文档需求状态括注违规。
- **待办（三裁决后更新）**：(1) ~~阶段2 整体效果确认与是否进入阶段3 待用户确认~~ 已确认（2026-08-20 用户批准进入阶段3 并开工 3.1；批次7+8+9 已入库：0083ad1、30b4c5e 及第三提交）；(2) 死代码 4 文件（DetectionControlPanel/RightPanelWidget/DeviceStatusPanel/SituationView，不在任何 CMake target）处置另立任务（用户已裁决本轮保持现状）；(3) ~~预存在紧急停止文档过期修正~~ 已完成（见入库记录）；(4) ~~GlobalStyle.h 9 个未入表 token（批次6 A5 完备性缺口，曾留阶段2）补登记 design-system.md~~ 已完成（2026-08-20，阶段3）；(5) xcb/Windows 真机三视口复核（可选，随用户侧构建）；(6) video_render_capture/stop_select_flicker_repro 既有失败（PatchCore 零检出）为主干侧探测问题，处置随主干另行安排，不属本计划范围。
