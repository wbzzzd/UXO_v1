...
| **撰写人** | 产品经理 Alice |
| **项目** | UXO_v1 排弹抢修指挥系统 |
| **关联 PRD** | [prd-mos.md](../requirements/prd-mos.md) V2.1（MOS 规划 PRD，Draft） |
| **技术栈** | Qt 5 / CMake / C++17 / QWidget + QPainter 自绘 |
| **范围说明** | 本文档**仅**覆盖 MOS（最小应急起降带规划）功能的产品设计。UXR（未爆弹识别）部分参见 [prd-uxr.md](../requirements/prd-uxr.md) |
...
> **UI 入口契约说明**：MOS 工作区的导航入口与逐控件契约现由 `docs/ui/pages/decision.md`（决策页）统一持有，复用现有"决策"导航项（`DEC-NAV-03`），不新增"规划"按钮。下方"新增规划按钮 / QStackedWidget page_index"方案为本文档早期历史设想，与 decision.md 不一致，仅作来源资料保留，不作为实现依据。

以下布局是早期来源草案，当前不作为 UI 契约。现有布局结构为：
...
| 3 | 殉爆安全系数 `K` | `QDoubleSpinBox` | 1.5 | 0.5~5.0 | ✅ | 模拟安全距离公式 `D=K×W^(1/3)` 中的系数；`W` 为模拟 TNT 当量（kg，默认取标称当量加不确定度）（示例值，待业务确认） |
...
| 8 | 释放面积增量权重 *(P1·MOS-004)* | `QDoubleSpinBox` | 0.70 | 0.00~1.00 | P1 | 修复优先级排序权重1：修复后释放的可起降面积增量（P1 启用，示例值） |
| 9 | 离轴线距离权重 *(P1·MOS-004)* | `QDoubleSpinBox` | 0.30 | 0.00~1.00 | P1 | 修复优先级排序权重2：离跑道中心轴线距离（P1 启用，权重1+权重2 应=1.0） |
...
| 12 | 候选排序数量 *(P1·MOS-010)* | `QSpinBox` | 3 | 1~10 | P1 | 每个档位输出的 Top-N 候选 MOS 矩形（P1 启用；P0 仅输出 1 个最大空矩形） |

**权重约束**：释放面积增量权重 + 离轴线距离权重 应始终 = 1.0，UI 可联动校验（修改一个时自动调整另一个）。该权重属 P1（MOS-004 修复优先级排序），P0 参数面板不含权重项。
...
**坐标系统**（与 [prd-mos.md](../requirements/prd-mos.md) MOS-001 对齐）：
- 跑道坐标系：原点为跑道一端中心，x∈[0,3000] 沿跑道长度方向（→），50m 道面带 y∈[-25,25]，500m 损毁分布核心区 y∈[-250,250]（y 向用带符号值，原点居中而非左下角）
...
│     │                  UXO:   K × (nominalYield + uncertainty)^(1/3)
    │     ├── id         = TargetInfo.id
    │     └── source     = "模拟" / "传感器"
    │
    └── DamagePointList
...
> **注**：上方 NavigationWidget "规划"按钮方案为本文档早期设想；MOS UI 入口与逐控件契约现由 [docs/ui/pages/decision.md](../ui/pages/decision.md) 持有（复用"决策"导航项），不新增"规划"按钮。

### 8.4 模拟边界与确定性约束

与 [prd-mos.md](../requirements/prd-mos.md) 安全边界声明对齐，本设计仅描述本地模拟功能：

- MOS 规划基于本地模拟损毁数据，UI 一律标注"模拟规划结果"或"模拟估算"，不接入真实传感器、不执行真实排弹或抢修动作。
- `mos_params.json`、`mos-demo-scenario.json` 等为本地模拟场景 fixture JSON，是本地模拟输入，不是运行态持久化；MVP 不写入任何运行态数据库。
- MOS-002/MOS-003 同一输入结果确定可复现，面积相等的候选按确定性规则打破并列（先按 y 起点升序、再按 x 起点升序）；P0 仅轴对齐搜索，不支持斜向（斜向列为 P1 MOS-016）。
- 模拟损毁分布生成器（MOS-015）按可配置随机种子确定性生成，同一种子与输入产生同一分布。
...
> **本节为非权威历史来源**：以下人天估算、里程碑分解与任务清单是本文档早期设想，不作为实现计划依据。权威优先级分期以 [prd-mos.md](../requirements/prd-mos.md) P0/P1/P2 分配为准（P0 = MOS-001/002/003/005/006/007/015；P1 = MOS-004/008/009/010/011/012/016；P2 = MOS-013/014）。下方原表中 P0 含"斜向枚举"与"修复优先级排序 MOS-004"两项与 PRD 现状冲突（P0 仅轴对齐搜索，MOS-004 已移至 P1），保留原表仅作历史对照，不作为实现依据。

### 10.1 历史 P0 设想（不构成“两周必须交付”）

| # | 需求 | 对应 PRD 编号 | 核心交付物 | 估算（人天） |
|---|------|-------------|-----------|:-----------:|
| 1 | 跑道与损毁模型 | MOS-001 | `RunwayModel` + `DamagePoint` 数据结构 + JSON 加载 | 2 |
| 2 | 窗口扫描法（平行 ~~+ 斜向枚举~~） | MOS-002 | ~~完整 MOS 搜索算法，含平行优先 + ±5°斜向枚举~~ + 碰撞检测。**注：P0 仅轴对齐（平行）搜索，Y 轴 1m 步长在 50m 道面带（y∈[-25,25]）上 50 个采样点，并列候选按确定性规则打破；斜向搜索列为 P1 MOS-016** | 5 |
| 3 | 多方案递进框架 | MOS-003 | 2~5 档位可变、每档按人工指定或简单规则选择修复点后重算 MOS、单调性验证 | 4 |
| 4 | 修复优先级排序 *(P1·MOS-004)* | MOS-004 | 加权排序（面积增量 70% + 离轴线 30%）、权重可配置。**注：MOS-004 已移至 P1，P0 每档修复哪些障碍物由人工指定或简单规则** | 3 |
...
> ~~P0 合计 26 人天 vs PRD 原估 22 人天，差额来自：文档新增的窗口扫描法斜向枚举复杂度（+2d）、完整的 2D View 自绘基础设施（+1d）、NavigationWidget 集成（+1d）。~~ **注：本估算是早期设想，其中"斜向枚举"在 P0 已移除（仅轴对齐），实际 P0 范围与估算以 [prd-mos.md](../requirements/prd-mos.md) 为准；本行保留仅作历史对照，不作为实现依据。**

### 10.2 历史 P1 设想（不构成已批准迭代）
