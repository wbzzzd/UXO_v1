...
| **撰写人** | 产品经理 Alice |
| **项目** | UXO_v1 排弹抢修指挥系统 |
| **关联 PRD** | [prd-mos.md](../prd/prd-mos.md) V2.1（MOS 规划 PRD，Draft） |
| **技术栈** | Qt 5 / CMake / C++17 / QWidget + QPainter 自绘 |
| **范围说明** | 本文档**仅**覆盖 MOS（最小应急起降带规划）功能的产品设计。UXR（未爆弹识别）部分参见 [prd-uxr.md](../prd/prd-uxr.md) |
...
**坐标系统**（与 [prd-mos.md](../prd/prd-mos.md) MOS-001 对齐）：
...
与 [prd-mos.md](../prd/prd-mos.md) 安全边界声明对齐，本设计仅描述本地模拟功能：
...
> **本节为非权威历史来源**：以下人天估算、里程碑分解与任务清单是本文档早期设想，不作为实现计划依据。权威优先级分期以 [prd-mos.md](../prd/prd-mos.md) P0/P1/P2 分配为准（P0 = MOS-001/002/003/005/006/007/015；P1 = MOS-004/008/009/010/011/012/016；P2 = MOS-013/014）。下方原表中 P0 含"斜向枚举"与"修复优先级排序 MOS-004"两项与 PRD 现状冲突（P0 仅轴对齐搜索，MOS-004 已移至 P1），保留原表仅作历史对照，不作为实现依据。
...
> ~~P0 合计 26 人天 vs PRD 原估 22 人天，差额来自：文档新增的窗口扫描法斜向枚举复杂度（+2d）、完整的 2D View 自绘基础设施（+1d）、NavigationWidget 集成（+1d）。~~ **注：本估算是早期设想，其中"斜向枚举"在 P0 已移除（仅轴对齐），实际 P0 范围与估算以 [prd-mos.md](../prd/prd-mos.md) 为准；本行保留仅作历史对照，不作为实现依据。**
