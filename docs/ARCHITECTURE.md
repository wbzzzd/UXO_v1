...
- 不引入分布式部署、消息总线、微服务或独立数据库进程。ZeroMQ 与 PostgreSQL 仅在 `CMakeLists.txt` 中以 `find_package(... QUIET)` 探测，当前未链接到任何目标，未在源码中使用；MQTT 只存在于设计资料和配置占位中。
...
本节描述期望架构，作为后续迁移的参照。除明确标注“已实现，保持”的 `SimulationWorkflow` 等现有组件外，本节新增的层次、目录、接口和模块名称均为设计占位。实现前必须先在 `.omo/plans/` 下落地计划并经用户确认。
