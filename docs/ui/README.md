...
早期参考图中的"紧急停止"在 CURRENT 源码里是恒禁用占位按钮：标注"紧急停止（模拟占位）"、tooltip"模拟占位，无实际效果"，点击无响应；源码保留的确认框槽 `onEmergencyStop` 因按钮恒禁用不可达，`MainWindow` 亦无信号消费者，不会停止任何设备。原型与 CURRENT 一致按禁用占位处理，详见 `pages/situation.md` 的状态栏章节与 `application-shell.md` 的遗漏与禁用清单。
...
CURRENT Qt 客户端已完成三视口 fresh 像素验证（REQ-011，2026-08-26）：态势/探测/决策三页 × 1280×720/1920×1080/3840×2160 共 9 张离屏截图留存于 `.omo/evidence/qt-viewport-adaptation/`；1280×720 三页无溢出与意外裁切（探测页左表格水平滚动条为用户裁决接受项，决策页参数区部分标签换行为既有登记的紧凑可用表现），1920×1080 与修复前基线零回归（动画帧差异除外），3840×2160 固定 token 区域与字体不缩放。此前“态势页右面板旧 `DecisionSuggestionPanel` 约 5px 底部溢出”的登记系失实记录（该组件未纳入 CMake 编译目标，见 `UI.md` 第 3 节注），已撤销；上表三视口约束仍是 Qt 实现阶段的目标规则。
