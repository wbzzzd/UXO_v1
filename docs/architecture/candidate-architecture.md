...
> 本文从 [ARCHITECTURE.md](../ARCHITECTURE.md) 第 9 节迁出，是架构细节子文档。NEXT 是已确认产品边界但尚未批准实施的草案，全部对应 [PRODUCT.md](../PRODUCT.md) 第 9 节中状态为 `Draft` 的 REQ-001 至 REQ-006。NEXT 必须先通过架构与 UI 设计评审并获用户批准，才能进入实现计划；本文为候选架构责任，不等于已批准实施。NEXT 是从 CURRENT 向长期 TARGET 演进的首个迁移切片，不是长期目标本身。
