...
每个动作执行前后，worker 捕获快照（状态、目标 ID、日志行数、标签页索引）并运行 13 条检查：
...
状态空间：5 个状态 × 10 个动作 = 50 个对。

### 4.4 HTML 仪表盘内容

- 顶部统计卡片：总轮数、崩溃数、卡死数、问题数
- 动作覆盖率柱状图：10 种动作各自的执行次数
- 唯一问题表：去重后的所有问题
...
**时间**：2026-07-30 16:24–16:33（约 10 分钟）
...
| 总问题数 | 0 |
| 动作执行总数 | ~2640 |
| 覆盖率 | 50/50（状态-动作对全部探索到）|

**结论**：在 132 轮无人值守、状态-动作全覆盖的随机交互下，UXO 客户端未出现崩溃、卡死或自洽性违规。扩展动作集（相机、标签页、刷新）和全部四个状态机迁移均表现稳定。
