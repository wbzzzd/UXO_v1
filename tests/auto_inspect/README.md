...
每个动作执行前后，worker 捕获快照（状态、目标 ID、日志行数、标签页索引）并运行 20 条检查：
...
**时间**：2026-08-03（A1 菜单关闭延迟扩展验证，3 轮 × 100 动作）
...
**新增动作 `menu_hover_timing`**：3 轮中共 10 次触发，测量菜单 Esc 关闭延迟均为 0ms（无延迟），R20 未报警。
