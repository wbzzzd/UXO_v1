...
每个动作执行前后，worker 捕获快照（状态、目标 ID、日志行数、标签页索引）并运行 29 条检查：
...
**时间**：2026-08-03（G 系列长期稳定性扩展验证，3 轮 × 100 动作）
...
**新增长期稳定性检查**：G 系列新增 R29 规则，为会话级检查（非 per-action），在主循环结束后调用一次。记录主循环开始前主窗口 `children().size()` 作为基线，循环结束后比较最终值，若增长超过阈值 +30 则报警（孤儿对话框/定时器累积未清理）。阈值 +30 允许正常波动（菜单/对话框短暂创建）。

**验证结果**：3 轮 × 100 动作全部通过（0 issues），说明 100 动作规模下无显著对象累积。R29 作为长期稳定性守护规则，未来若长时间运行（如 run_loop.sh 持续数百轮）出现内存泄漏或对象累积会自动报警。

**历史扩展**：A 系列（R20-R23 性能时序）、D 系列（R24/R25 对话框守护）、F 系列（R26 输入鲁棒性）、C 系列（R27 标签页视觉）、E 系列（R28 业务流自洽性）均已在之前批次验证通过，本次 G 系列验证未触发回归。
