...
每个动作执行前后，worker 捕获快照（状态、目标 ID、日志行数、标签页索引）并运行 18 条检查：
...
状态空间：5 个状态 × 17 个动作 = 85 个对。

### 4.4 HTML 仪表盘内容

- 顶部统计卡片：总轮数、崩溃数、卡死数、问题数
- 动作覆盖率柱状图：17 种动作各自的执行次数
- 唯一问题表：去重后的所有问题
...
**时间**：2026-08-03（Layer 3 扩展验证，20 轮 × 25 动作）
...
| 总问题数 | 0 |
| 动作执行总数 | 500 |
| 覆盖率 | 85/85（5 状态 × 17 动作全部探索到）|

**新增动作覆盖**（20 轮内）：`mission_row`、`device_row`、`status_tab` 均被探索到。

**结论**：在 Layer 3 扩展（任务/设备表行点击、状态子标签页点击）下，UXO 客户端未出现崩溃、卡死或自洽性违规。任务表/设备表行点击不破坏目标选中状态（R17 通过），状态子标签页点击不破坏选中状态（R18 通过）。
