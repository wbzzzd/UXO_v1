...
每个动作执行前后，worker 捕获快照（状态、目标 ID、日志行数、标签页索引）并运行 16 条检查：
...
状态空间：5 个状态 × 14 个动作 = 70 个对。

### 4.4 HTML 仪表盘内容

- 顶部统计卡片：总轮数、崩溃数、卡死数、问题数
- 动作覆盖率柱状图：14 种动作各自的执行次数
- 唯一问题表：去重后的所有问题
...
**时间**：2026-08-03（Layer 1 扩展验证，20 轮 × 25 动作）
...
| 总问题数 | 0 |
| 动作执行总数 | 500 |
| 覆盖率 | 70/70（5 状态 × 14 动作全部探索到）|

**新增动作覆盖**（20 轮内）：`menu_action` 34 次、`view_toggle` 22 次、`search_input` 34 次、`nav_button` 34 次。

**结论**：在 Layer 1 扩展（菜单/视图切换/搜索框/导航按钮遍历）下，UXO 客户端未出现崩溃、卡死或自洽性违规。菜单弹出的 QMessageBox 由 worker 预设的 `QTimer` 自动关闭，未造成卡死。视图切换（面板隐藏/显示）恢复后状态保持，搜索框输入不破坏选中状态。
